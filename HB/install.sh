...
sudo apt-get install -y firefox htop git python2.7-dev libxml2-dev libxslt-dev libffi-dev libssl-dev build-essential xvfb libboost-python2.7-dev libleveldb-dev libjpeg-dev curl wget
...
# python2.7 requirements are already installed by .travis.yml on Travis
if [ "$TRAVIS" != "true" ]; then
