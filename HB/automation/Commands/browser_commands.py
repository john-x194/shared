...
def get_website(url, sleep, visit_id, webdriver,
                browser_params, extension_socket, proxy=None, server=None):
    """
    ...
    # write_out_har = False
    # with open('/home/johncook/headerBidding/TrackingProject/src/write_out_har.json') as f: 
    #     write_out_har = json.load(f)['writing']
    # intent_url_found = ""
    # # print('write out har {}'.format(write_out_har))
    # if write_out_har: 
        # try: 
        #     for intent_url in intent_check:
        #         # print(intent_url, url, intent_url in url)
        #         if intent_url in url: 
        #             intent_url_found = intent_url.split('/')[0]+'_INTENT_URL'
        #     if intent_url !="":
        #         proxy.new_har("har_" + str(intent_url_found), options={'captureHeaders': True, 'captureContent': True})
        #     else: 
        #         proxy.new_har("har_" + str(url), options={'captureHeaders': True, 'captureContent': True})
        # except Exception as e:
        #     print('Exception {} occured during processing url {}'.format(e, url)) 
    ...
    # if write_out_har: 
    #     if intent_url_found != "": 
    #         path = '/mnt/hgfs/archive_save/hars/HAR_{}.json'.format(intent_url_found)
    #     else: 
    #         path = '/mnt/hgfs/archive_save/hars/HAR_{}.json'.format(url.split('://')[1])
...
def browse_website(url, num_links, sleep, visit_id, webdriver,
    ...
    """I added this to capture HAR files using mob_proxy
    """
    # server = Server('/home/johncook/TrackingProject/src/browsermob-proxy-2.1.4/bin/browsermob-proxy')
    # server.start()
    # proxys = {}
    # proxy = server.create_proxy()
    # webdriver.firefox_profile.set_proxy(proxy.selenium_proxy())
    ...
    # path = '/mnt/hgfs/archive_save/hars/HAR_{}.json'.format(url.split('://')[1])
    # print("[AUTOMATION] {} Writing {} to HAR file".format(time.asctime(), path))
    # with open(path, 'w') as f: 
    #     json.dump(proxy.har, f)
